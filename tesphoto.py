...
def detect_emotion():
    ...
    for image_file in image_files:
        frame = cv2.imread(os.path.join('images', image_file))  # โหลดภาพจากโฟลเดอร์ "images"
        if frame is None:
            continue
        
        # ปรับขนาดภาพให้มีขนาดคงที่ (เช่น 640x480)
        ...
        if results.detections:
            ...
            for detection in results.detections:
                ...
                cv2.putText(frame_resized, emotion_label, (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, color, 2)
...
def index():
    ...
    <head>
        ...
        <style>
            ...
            #emotion-history { max-height: 400px; overflow-y: auto;}
